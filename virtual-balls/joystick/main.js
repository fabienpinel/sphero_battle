...
(
    function () {
        document.querySelector('#login-form form').addEventListener('submit', function (event) {
            ...
            if (!player) {
                var http = getAjax();
                http.open('POST', 'http://134.59.215.166:3000/api/players/', true);
...
(
    function () {
        ...
        function go (x, y) {
            if (x !== 0 && y !== 0 && player) {
                var http = getAjax();
                http.open('POST', 'http://134.59.215.166:3000/api/players/' + player.id + '/deltaX/' + x + '/deltaY/' + y, true);
                http.setRequestHeader("Content-type","application/json");
                http.send();
            }
        }
        ...
        function _castSpell() {
            if (player) {
                var http = getAjax();
                http.open('POST', 'http://134.59.215.166:3000/api/players/' + player.id + '/cast-spell', true);
                http.setRequestHeader("Content-type","application/json");
                http.onload = function (response) {
...
window.onbeforeunload = function () {
    if (player) {
        var http = getAjax();
        http.open('DELETE', 'http://134.59.215.166:3000/api/players/' + player.id, true);
        http.setRequestHeader("Content-type","application/json");
        http.send();
    }
};